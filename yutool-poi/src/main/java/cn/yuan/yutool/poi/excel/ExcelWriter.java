...
import cn.yuan.yutool.poi.excel.style.Align;
...
public class ExcelWriter extends ExcelBase<ExcelWriter> {
	 ...
	 */
	public ExcelWriter(String destFilePath) throws Exception {
	 ...
	 * @param sheetName sheet名，第一个sheet名并写出到此sheet，例如sheet1
	 */
	public ExcelWriter(String destFilePath, String sheetName) throws Exception {
		this(new File(destFilePath), sheetName);
	}
	 ...
	 */
	public ExcelWriter(File destFile) throws Exception {
		this(destFile, null);
	}
	 ...
	 */
	public ExcelWriter(File destFile, String sheetName) throws Exception {
	...
	public ExcelWriter merge(int lastColumn, Object content, boolean isSetHeaderStyle) {
		Assert.assertFalse( "ExcelWriter has been closed!",this.isClosed);
	...
	public ExcelWriter write(Iterable<?> data, Comparator<String> comparator) throws Exception {
		Assert.assertFalse( "ExcelWriter has been closed!",this.isClosed);
	 ...
	 */
	public ExcelWriter flush() throws Exception {
	 ...
	 * @since 1.0-jdk1.6
	 */
	public ExcelWriter flush(File destFile) throws Exception {
		Assert.assertNotNull( "[destFile] is null, and you must call setDestFile(File) first or call flush(OutputStream).",destFile);
		return flush(new FileOutputStream(destFile), true);
	}
	 ...
	 * @throws RuntimeException IO异常
	 */
	public ExcelWriter flush(OutputStream out) throws RuntimeException {
	 ...
	 * @since 4.4.1
	 */
	public ExcelWriter flush(OutputStream out, boolean isCloseOut) throws RuntimeException {
		Assert.assertFalse( "ExcelWriter has been closed!",this.isClosed);
		try {
